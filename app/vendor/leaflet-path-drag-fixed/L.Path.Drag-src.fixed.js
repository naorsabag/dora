//Fixed this library to work when inside an iframe, on the outer document.
//The problem was that the document object was used to set the drag event.
//The shape was dragged by mouse movement inside the iframe, instead of on the shape itself.
//The fix replaces the use of the document object, with accessing the _container DOM element of leaflet, and using its ownerDocument property.
...
L.Handler.PathDrag = L.Handler.extend( /** @lends  L.Path.Drag.prototype */ {
    ...
    _onDragStart: function(evt) {
        ...
        L.DomEvent
            .on(evt.target._map._container.ownerDocument, L.Draggable.MOVE[eventType], this._onDrag, this)
            .on(evt.target._map._container.ownerDocument, L.Draggable.END[eventType], this._onDragEnd, this);
    ...
    _onDragEnd: function(evt) {
        ...
        L.DomEvent
            .off(evt.view.document, 'mousemove touchmove', this._onDrag, this)
            .off(evt.view.document, 'mouseup touchend', this._onDragEnd, this);
...
L.Canvas.include({
    ...
    transformPath: function(layer, matrix) {
        ...
        if (!copy) { // get copy of all rendered layers
            copy = this._containerCopy = this._container.ownerDocument.createElement('canvas');
